...
@interface RDLinkedInAuthorizationController : UIViewController <UIWebViewDelegate> {
  id<RDLinkedInAuthorizationControllerDelegate> __unsafe_unretained rdDelegate;
  RDLinkedInEngine* rdEngine;
  UINavigationBar*  rdNavBar;
  UIWebView*        rdWebView;
}

@property (nonatomic, unsafe_unretained)   id<RDLinkedInAuthorizationControllerDelegate> delegate;
